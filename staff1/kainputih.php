...
<!-- kiri -->
      ...
      <button style="font-size: 14px; " type="button" class="btn btn-success" data-toggle="modal" data-target="#tambahModal">Tambah Data</button>
            <br>
          <br>
                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    if (isset($_POST['submit'])){
                        ...
                        while ($row = $result->fetch_object()){
                         ...
                         if (isset($row->Tanggal_masuk)){
                            if($row->Tanggal_keluar == ''){
                              echo "<td><a href='kainputih.php?user=$user&id=$row->id&tanggal=$row->Tanggal_masuk&jenis=$row->jenis&jumlah=$row->jumlah'><button type='button' class='btn btn-secondary'>Tulis</button></a></td>";
                            }
                            else{
                              echo "<td>$row->Tanggal_keluar</td>";
                            }
                        ...
                        require_once ('../database/db_login.php');
                        $query = "SELECT kp.id, kp.jenis,kp.jumlah,kp.Tanggal_masuk,kp.Tanggal_keluar FROM kain_putih kp order by kp.Tanggal_masuk desc";
                        $result = $db->query($query);
                        while ($row = $result->fetch_object()){
                          echo "<tr>";
                          echo "<th scope='row'>$row->id</th>";
                         ...
                         if (!isset($row->Tanggal_keluar )){echo "<td><a href='hapuskainputih.php?user=$user&id=$row->id'><button type='button' class='btn btn-danger'>Hapus</button></a></td>";}
                          else{
                            echo "<td></td>";
                          }
                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    if (isset($_POST['submit1'])){
                      ...
                      else if (isset($_POST['submitkeluar1'])){
                        require_once ('../database/db_login.php');
                        $query = "SELECT kpp.id, kpp.jenis_kain,kpp.jumlah,kpp.tanggal_masuk,kpp.tanggal_keluar FROM kain_putih_potong kpp where kpp.tanggal_keluar = '$tanggalK'";
                        $result = $db->query($query);
                        while ($row = $result->fetch_object()){
                          echo "<tr>";
                          echo "<th scope='row'>$row->id</th>";
                      ...
                      else if (isset($_POST ['submitmasuk1'])){
                        require_once ('../database/db_login.php');
                        $query = "SELECT kpp.id, kpp.jenis_kain, kpp.jumlah,kpp.tanggal_masuk,kpp.tanggal_keluar FROM kain_putih_potong kpp where kpp.tanggal_masuk = '$tanggalM'";
                        $result = $db->query($query);
                        while ($row = $result->fetch_object()){
                          echo "<tr>";
                          echo "<th scope='row'>$row->id</th>";
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Tambah Daftar Pembatik</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form id="form" method="post" action="tambahkain.php?user=<?php echo $user; ?>">
                    

                    <div class="form-group">
                    <label>id kain</label>
                    <input placeholder="Masukkan id kain" type="text" class="form-control" name="id" required>
                  </div>
                  <div class="form-group">
                    <label>Jenis Kain</label>
                    <input placeholder="Masukkan jenis kain" type="text" class="form-control" name="jeniskain" required>
                  </div> 
...
<!-- modal tulis tanggal masuk -->


    <div class="modal fade" id="tanggal_keluar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Tambah Daftar Pembatik</h5>
          ...
          <div class="modal-body">
            <form id="form" method="post" action="tambahtanggal.php?user=<?php echo $user; ?>&id=<?php echo $_GET['id']; ?>&tanggal=<?php echo $_GET['tanggal'] ?>&jenis=<?php echo $_GET['jenis'] ?>&jumlah=<?php echo $_GET['jumlah'] ?>">
              <!-- ukuran -->

              <div class="form-group">
                    <label>Ukuran Kain</label>
                    <input placeholder="Masukkan Ukuran Kain yang Sudah di Potong" type="number" class="form-control " name="ukuran" required>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <input placeholder="Masukkan Tanggal" type="date" class="form-control datepicker" name="tanggal_keluar" required>
                  </div>
                  <button type="submit" class="btn btn-primary" value="tambah">Tambah</button>
                </form>
          </div>
...
<!-- menampilkan modal pilih tanggal keluar -->
  <?php
        if(isset($_GET['id']) && isset($_GET['tanggal']) && isset($_GET['jenis']) && isset($_GET['jumlah']) ){
            ?>
            <script type="text/javascript">
                $("#tanggal_keluar").modal();
            </script>
    <?php
        }
    ?>

<?php
  ...
  $teks = 'Apakah Yakin ingin menghapus?';
?>
<!DOCTYPE html>
<html>
...
<body>
  ...
  <section id="container">
      ...
      <section class="wrapper">
        ...
        <div class="row" id="panel_cari">
          <div class="col-md-3 portlets">
            <div class="panel">
              <form method="POST" action="dataPembatikKoordinator.php?user=<?php echo $user?>">
                Cari Nama Pembatik :
        ...
        <button style="font-size: 14px; " type='button' class='btn btn-success' data-toggle="modal" data-target="#tambahModal">Tambah Pembatik</button><br/><br/>
                    ...
                    <?php
                      if (isset($pembatik)){
                        ...
                        while ($row = $result->fetch_object()){
                          ...
                          echo "<td><a href='dataPembatikKoordinator.php?user=$user&kode_pembatik=$row->kode_pembatik'><button style='font-size: 12px; ' type='button' class='btn btn-primary' data-toggle='modal' data-target='#editModal'>Ubah Kuota</button></a> <a href='hapusPembatik.php?user=$user&kode_pembatik=$row->kode_pembatik' onclick= 'return confirm()'><button style='font-size: 12px; ' type='button' class='btn btn-danger' data-toggle='modal' data-target='#hapusModal'>Hapus</button></a></td>";
                          echo "</tr>";
                        }
                      }
                      else {
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLabel">Tambah Pembatik</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body">
            <form id="form" method="post" action="tambahPembatik.php?user=<?php echo $user; ?>">
                  <div class="form-group">
                    <label>Kode Pembatik</label>
                    <input type="number" class="form-control" placeholder="Kode Pembatik" name="kode_pembatik" minlength="1" maxlength="4" required autofocus>
                  </div>
                  <div class="form-group">
                    <label>Nama Pembatik</label>
                    <input type="text" class="form-control" placeholder="Nama Pembatik" name="nama_pembatik" pattern="[a-zA-Z ]+" maxlength="20" required autofocus>
                  </div>
              <div class="form-group">
                    <label>Kuota</label>
    ...
    <div class="modal fade" id="edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form" method="post" action="editPembatik.php?user=<?php echo $user; ?>&kode_pembatik=<?php echo $_GET['kode_pembatik']; ?>">
              <div class="form-group">
                <?php
                $kode_pembatik = $_GET['kode_pembatik'];
                require_once ('../database/db_login.php');
                $query2 = "SELECT * FROM pembatik WHERE kode_pembatik='$kode_pembatik'";
                $result2 = $db->query($query2);
                while ($row = $result2->fetch_object()){
                      echo "<label>Ubah Kuota Pembatik : $row->nama_pembatik</label>";
                    }
                    ?>
                    <input type="number" class="form-control" placeholder="Kuota" name="kuota" minlength="1" maxlength="4" required autofocus>
              </div>
    ...
    <?php
        if(isset($_GET['user']) && isset($_GET['kode_pembatik'])){
            ?>

      ...
      <section class="wrapper">
        ...
        <div class="row" id="panel_cari">
          <div class="col-md-3 portlets">
            <div class="panel">
              <form method="POST" action="daftarPembatikKoordinator.php?user=<?php echo $user;?>">
                Cari Nama Pembatik :
        ...
        <div class="col-md-16 portlets">
            <div class="panel panel-default">
              <div class="panel-heading">
                <h2><strong>Daftar Pembatik</strong></h2>
              </div>
                  ...
                  <tbody>
                    ...
                    if ((isset($_POST['submitNama'])) || (isset($_POST['submitSekarang'])) || (isset($_POST['submitAmbil'])) || (isset($_POST['submitKembali']))){
                        ...
                        if ((isset($_POST['submitNama']))){
                            ...
                            $query = "SELECT d.oleh AS oleh, d.jenis_kain AS id, d.kode_motif AS kode_motif, d.kode_pembatik AS kode_pembatik, p.nama_pembatik AS nama, m.nama_motif AS nama_motif, d.jumlah_kain AS jumlah_kain, d.batas AS batas, d.tanggal_ambil AS tanggal_ambil, d.tanggal_kembali AS tanggal_kembali, k.jenis_kain AS nama_kain FROM daftar_pembatik d, motif m, pembatik p, kain_potong k WHERE d.jenis_kain = k.id AND d.kode_motif = m.kode_motif AND d.kode_pembatik = p.kode_pembatik d.batas = '$tanggal_sekarang' ORDER BY d.batas desc";
                        }
                        ...
                        while ($row = $result->fetch_object()){
                          ...
                          echo "<td>$row->oleh</td>";
                          echo "<td><a href='hapusDaftarPembatik.php?user=$user&pembatik=$row->kode_pembatik&tanggal=$row->batas&motif=$row->kode_motif&jumlah=$row->jumlah_kain&jenis_kain=$row->id' onclick= 'return confirm()'><button style='font-size: 12px; ' type='button' class='btn btn-danger'>Hapus</button></a></td>";
                          echo "</tr>";
                        }
                        echo "</tbody>";
                        echo "</table>";
                        ...
                        $no =$mulai+1; 
                        //$query = "SELECT d.kode_motif AS kode_motif, d.kode_pembatik AS kode_pembatik, p.nama_pembatik AS nama, m.nama_motif AS nama_motif, d.jumlah_kain AS jumlah_kain, d.batas AS batas, d.tanggal_ambil AS tanggal_ambil, d.tanggal_kembali AS tanggal_kembali FROM daftar_pembatik d, motif m, pembatik p WHERE d.kode_motif = m.kode_motif AND d.kode_pembatik = p.kode_pembatik ORDER BY d.batas desc LIMIT $mulai, $halaman";
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="form" method="post" action="tambahDaftarPembatik.php?user=<?php echo $user; ?>">
                  <div class="form-group">
                    <label>Pilih Pembatik</label>
                    <select name="pembatik" class="form-control" required>
                        ...
                        <?php
                          ...
                          $result = $db->query($query);
                          while ($row = $result->fetch_object()){
                            echo "<option value='$row->kode_pembatik'>$row->nama_pembatik</option>";
                          }
                        ?>
					</select>
                  </div>
                  <div class="form-group">
                    ...
                    <select name="motif" class="form-control" required>
                        ...
                        <?php
                          ...
                          $result = $db->query($query);
                          while ($row = $result->fetch_object()){
                            echo "<option value='$row->kode_motif'>$row->nama_motif</option>";
                          }
                        ?>
					</select>
                  </div>
                  <div class="form-group">
                    ...
                    <select name="jenis_kain" class="form-control" required>
						<option value="">Pilih Jenis Kain</option>
                        <?php
                          require_once ('../database/db_login.php');
                          $query = "SELECT * FROM kain_potong";
                          $result = $db->query($query);
                          while ($row = $result->fetch_object()){
                            echo "<option value='$row->id'>$row->jenis_kain (ukuran : $row->ukuran m x m)</option>";
                          }
                        ?>
					</select>
                  </div>

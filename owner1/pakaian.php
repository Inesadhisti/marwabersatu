<?php
  ...
  if(isset($_POST['tglkeluar'])){ //ambil=keluar
    ...
    $Tanggal_keluar = date('Y-m-d', strtotime($tglAmbil));
  }
                  ...
                  <tbody>
                    <?php
                      if (isset($_POST['submit'])){
                        ...
                        require_once ('../database/db_login.php');
                        $query = "SELECT p.id_pakaian as id_pakaian,p.model as model,p.motif as motif,p.jumlah as jumlah,p.tanggal_masuk as tanggal_masuk,p.tanggal_keluar as tanggal_keluar FROM pakaian p order by p.tanggal_masuk desc";
                        $result = $db->query($query);
                        while ($row = $result->fetch_object()){
                          echo "<tr>";
                          echo "<th scope='row'>$row->id_pakaian</th>";

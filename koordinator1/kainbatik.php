<?php
  ...
  if(isset($_POST['Tanggal_keluar'])){
    ...
    $tanggalK = date('d-m-Y', strtotime($TanggalK));
  }
  else if(isset($_POST['Tanggal_masuk'])){
    $TanggalM = $_POST['Tanggal_masuk'];
    $tanggalM = date('d-m-Y', strtotime($TanggalM));
  }

  else if(isset($_POST ['nama_motif'])) {
          ...
          <div class="col-md-3 portlets">
            <div class="panel">
              <form method="POST" action="kainbatik.php?user=<?php echo $user?>">
                Cari Nama Motif :
                <input placeholder="Masukkan kain batik yang di cari" type="text" class="form-control" name="nama_motif">
                ...
                <table class="table">
                  ...
                  <tbody>
                    ...
                    if (isset($_POST['submit'])){
                        ...
                        $query = "SELECT d.Kode_motif AS Kode_motif, d.Nama_motif AS Nama_motif, d.Jumlah AS jumlah,  d.Tanggal_masuk AS Tanggal_masuk, d.Tanggal_keluar AS Tanggal_keluar FROM kain_batik d WHERE d.Nama_motif = '$nama_motif'";
                        ...
                        require_once ('../database/db_login.php');
                        $query = "SELECT d.Kode_motif AS Kode_motif, d.Nama_motif AS Nama_motif, d.Jumlah AS jumlah,  d.Tanggal_masuk AS Tanggal_masuk, d.Tanggal_keluar AS Tanggal_keluar FROM kain_batik d WHERE d.Tanggal_keluar = '$tanggalK'";
                        $result = $db->query($query);
                        while ($row = $result->fetch_object()){
                          echo "<tr>";
                          echo "<th scope='row'>$row->Kode_motif</th>";
                        ...
                        require_once ('../database/db_login.php');
                        $query = "SELECT d.Kode_motif AS Kode_motif, d.Nama_motif AS Nama_motif, d.Jumlah AS jumlah,  d.Tanggal_masuk AS Tanggal_masuk, d.Tanggal_keluar AS Tanggal_keluar FROM kain_batik d WHERE d.Tanggal_masuk = '$tanggalM'";
                        $result = $db->query($query);
                        while ($row = $result->fetch_object()){
                          echo "<tr>";
                          echo "<th scope='row'>$row->Kode_motif</th>";
